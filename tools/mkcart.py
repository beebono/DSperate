...
The stubs here only spin: the point is to be tapped, not to run. The launch is
visible to the emulator as the first cart read at arm9_rom_offset, which nothing
else in a session touches.
...
def icon_from_image(path):
    ...
    palette = [0] + [rgb555(raw[i * 3], raw[i * 3 + 1], raw[i * 3 + 2]) for i in range(15)]
...
def build_banner(px, palette, title):
    ...
    for ty in range(4):                            # 8x8 tiles, row-major
        for tx in range(4):
            base = 0x20 + (ty * 4 + tx) * 32
            for row in range(8):
                for col in range(0, 8, 2):
                    lo = px[ty * 8 + row][tx * 8 + col]
                    hi = px[ty * 8 + row][tx * 8 + col + 1]
                    ban[base + row * 4 + col // 2] = lo | (hi << 4)
