        ...
        if any(counts):
            ...
            if args.png and pngs < args.max_png:
                for s, name in enumerate(('top', 'bottom')):
                    ...
                    a = fr[s * SCREEN:(s + 1) * SCREEN]; b = fc[s * SCREEN:(s + 1) * SCREEN]
                    write_png(os.path.join(args.png, f'f{f:04d}_{name}_ref.png'), a)
                    write_png(os.path.join(args.png, f'f{f:04d}_{name}_cand.png'), b)
                    write_png(os.path.join(args.png, f'f{f:04d}_{name}_diff.png'), diff_image(a, b))
                pngs += 1
