...
//
// Thumb-state executor (Thumb v2 on the ARM9: BLX, BKPT).
...
u32 exec_thumb(CpuContext& cpu, u16 instr) {
  ...
  case TOp::Alu: {
    ...
    bool c = carry(cpu);
    switch ((instr >> 6) & 0xF) {
    case 0x0: r = a & b; R(cpu, rd) = r; set_nz(cpu, r); return 0;                 // AND
    case 0x1: r = a ^ b; R(cpu, rd) = r; set_nz(cpu, r); return 0;                 // EOR
    case 0x2: r = shift_reg(0, b, a, c); R(cpu, rd) = r; set_nzc(cpu, r, c); cpu.cycle_class = CYC_CI; return 1;   // LSL
    case 0x3: r = shift_reg(1, b, a, c); R(cpu, rd) = r; set_nzc(cpu, r, c); cpu.cycle_class = CYC_CI; return 1;   // LSR
    case 0x4: r = shift_reg(2, b, a, c); R(cpu, rd) = r; set_nzc(cpu, r, c); cpu.cycle_class = CYC_CI; return 1;   // ASR
    case 0x5: R(cpu, rd) = add_flags(cpu, a, b, c); return 0;                      // ADC
    case 0x6: R(cpu, rd) = sub_flags(cpu, a, b, c); return 0;                      // SBC
    case 0x7: r = shift_reg(3, b, a, c); R(cpu, rd) = r; set_nzc(cpu, r, c); cpu.cycle_class = CYC_CI; return 1;   // ROR
    case 0x8: r = a & b; set_nz(cpu, r); return 0;                                 // TST
    case 0x9: R(cpu, rd) = sub_flags(cpu, 0, b, 1); return 0;                      // NEG
    case 0xA: sub_flags(cpu, a, b, 1); return 0;                                   // CMP
    case 0xB: add_flags(cpu, a, b, 0); return 0;                                   // CMN
    case 0xC: r = a | b; R(cpu, rd) = r; set_nz(cpu, r); return 0;                 // ORR
    case 0xD: {                                                                    // MUL
      r = a * b; R(cpu, rd) = r; set_nz(cpu, r); cpu.cycle_class = CYC_CI;
      if (is_arm9(cpu)) return 3;
    ...
    case 0xE: r = a & ~b; R(cpu, rd) = r; set_nz(cpu, r); return 0;                // BIC
    default:  r = ~b; R(cpu, rd) = r; set_nz(cpu, r); return 0;                    // MVN
    }
  }
  case TOp::HiRegOp: {
    ...
    switch ((instr >> 8) & 3) {
    case 0:
      if (rd == 15) { cpu.jump(R(cpu, 15) + b, false); return 0; }
      R(cpu, rd) += b; return 0;
    case 1: sub_flags(cpu, R(cpu, rd), b, 1); return 0;
    case 2:
      if (rd == 15) { cpu.jump(b, false); return 0; }
      R(cpu, rd) = b; return 0;
    default: return 0;
    }
  }
  ...
  case TOp::LdrStrReg: {
    ...
    switch ((instr >> 9) & 7) {
    case 0: mem_write32(cpu, ea, R(cpu, rd)); return 0;                         // STR
    case 1: mem_write16(cpu, ea, static_cast<u16>(R(cpu, rd))); return 0;      // STRH
    case 2: mem_write8(cpu, ea, static_cast<u8>(R(cpu, rd))); return 0;        // STRB
    case 3: R(cpu, rd) = static_cast<u32>(static_cast<s32>(static_cast<s8>(mem_read8(cpu, ea)))); return 0;  // LDRSB
    case 4: R(cpu, rd) = rotr32(mem_read32(cpu, ea), (ea & 3) * 8); return 0;  // LDR
    case 5: { u32 v = mem_read16(cpu, ea); if (!is_arm9(cpu) && (ea & 1)) v = rotr32(v, 8); R(cpu, rd) = v; return 0; } // LDRH
    case 6: R(cpu, rd) = mem_read8(cpu, ea); return 0;                         // LDRB
    default:                                                                    // LDRSH
  ...
  case TOp::LdrStrImm5: {
    ...
    if (b) {
      ...
      if (l) { R(cpu, rd) = mem_read8(cpu, ea); return 0; }
      mem_write8(cpu, ea, static_cast<u8>(R(cpu, rd))); return 0;
    }
    const u32 ea = R(cpu, rb) + imm * 4;
    if (l) { R(cpu, rd) = rotr32(mem_read32(cpu, ea), (ea & 3) * 8); return 0; }
    mem_write32(cpu, ea, R(cpu, rd)); return 0;
  }
  case TOp::LdrStrHImm5: {
    const u32 rb = (instr >> 3) & 7, rd = instr & 7, ea = R(cpu, rb) + ((instr >> 6) & 0x1F) * 2;
    if (instr & (1 << 11)) { u32 v = mem_read16(cpu, ea); if (!is_arm9(cpu) && (ea & 1)) v = rotr32(v, 8); R(cpu, rd) = v; return 0; }
    mem_write16(cpu, ea, static_cast<u16>(R(cpu, rd))); return 0;
  }
  case TOp::LdrStrSpRel: {
    const u32 rd = (instr >> 8) & 7, ea = R(cpu, 13) + (instr & 0xFF) * 4;
    if (instr & (1 << 11)) { R(cpu, rd) = rotr32(mem_read32(cpu, ea), (ea & 3) * 8); return 0; }
    mem_write32(cpu, ea, R(cpu, rd)); return 0;
  }
  case TOp::AddPcSp: {
  ...
  case TOp::StmLdm: {
    ...
    if (list == 0) {                        // empty list: r15, base += 0x40
      if (load) cpu.jump(mem_read32(cpu, addr), false);
      else mem_write32(cpu, addr, R(cpu, 15) + 2);
      R(cpu, rb) = addr + 0x40;
      return 0;
    }
  ...
  case TOp::BCond: {
    ...
    if (!arm::check_condition(cond, cpu.hot.cpsr)) return 0;
    s32 off = static_cast<s8>(instr & 0xFF) * 2;
    cpu.jump(R(cpu, 15) + static_cast<u32>(off), false);
    return 0;
  }
  case TOp::Swi: cpu.raise_exception(CpuContext::Exception::Swi); return 0;
  case TOp::Bkpt:
    if (!is_arm9(cpu)) break;
