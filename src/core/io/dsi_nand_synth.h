...
// Micro Hei, generated by tools/make_dsi_font.py) for a console of `region`
// (DsiRegion::region). China (4) and Korea (5) have tables of their own: nine
// resource entries with theirs at 3-5 or 6-8, which is where their titles'
// SDK looks; every other region has the normal three. The first 0x80 bytes
// are a marker where Nintendo's RSA signature would be; titles check that
// signature through DSi BIOS SWI 22h, which NDS::dsi_hle_swi answers for the
// marker.
std::vector<u8> builtin_dsi_font(u8 region = 1);
bool is_builtin_font_signature(const u8 sig[0x80]);
// Which console a TWLFontTable.dat is for: 0 the normal table, 4 China,
// 5 Korea, -1 not one of those layouts.
