...
# oc_dma = false                # Timing OC, DMA half: DMA units cost no cycles. MEASURED WORSE in every
                                # combination (Golden Sun +35% alone, +8% on top of no_fifo): it overclocks
                                # the guest, which spends the cycles. Do not enable.
# oc_gx = false                 # Timing OC, geometry half: geometry commands cost no cycles. Flat alone;
                                # on top of no_fifo another -2% on Golden Sun (frames still identical
                                # there) but it permanently desyncs Dragon Ball Origins' intro.
...
# layout_prev screen_swap pip_corner_next fullscreen screenshot lid mic (held). A value is a key name, or
