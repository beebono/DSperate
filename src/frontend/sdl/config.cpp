...
#                               # Only used on a firmware boot: launching the card in the DS menu
#                               # raises this list, and picking a game boots it. Unset, the picker
#                               # says so rather than showing an empty box.
# saves = /path/to/saves        # battery saves; default next to the ROM
# cache = /path/to/cache        # where a zipped game is unpacked to when it cannot be unpacked
#                               # beside its zip (a read-only card, say). Zips are unpacked once,
...
[loader]
# The card the DS menu shows on a firmware boot. It is built into the emulator,
# so nothing has to be shipped beside it; a BootMenu.nds next to this file
# replaces it entirely, which is how a card with your own icon from
# tools/mkcart.py is used.
# title = Game Menu             # first banner line
# subtitle = Dariragan! Dagozuban!   # second line; empty for a one-line banner
...
#                               # where SDL has no display to draw on (no X/Wayland, no DRM) but
#                               # fb0 answers, or where the mali driver is named.
# vsync = true
...
[cart]
# cache_mb = 2048               # the most unpacked zipped games kept per directory, in MB; when a
#                               # new one would not fit, the least recently launched go first
#                               # (never the one being launched). 0 = keep everything. Images whose
#                               # zip has been deleted or moved are removed on the next launch.
# cache = keep                  # keep: unpacked games stay for the next launch (the default: an
#                               # unpack is seconds to a minute on a card) | session: deleted when
...
# autosave_png = false          # a thumbnail with the auto-save: laid out as the window shows them, RGBA.
#                               # true writes <states>/<GAMECODE>.auto.png beside the state; a path
#                               # writes that file instead (a launcher can name the picture its game
#                               # switcher shows). Taken from the emulator's own frame, so it is right
#                               # on panels a screen grabber cannot read (hardware scaler layers).
